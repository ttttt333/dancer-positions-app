...
type Params = {
  ...
  playbackDancers: unknown;
  previewDancers: unknown;
...
/**
 * ステージ枠ハンドルによる W/D mm リサイズ（ドラッグ draft → ポインタアップで確定）。
 */
export function useStageBoardStageResize({
  setProject,
  viewMode,
  ...
  const [stageResizeDraft, setStageResizeDraft] =
    useState<StageResizeDraft | null>(null);

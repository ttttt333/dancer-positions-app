...
const WAVE_CHROME_TITLE =
  "波形上でマウスホイール（またはトラックパッドの縦スクロール）で時間軸の拡大・縮小。上の秒数目盛りをクリックすると再生位置だけ移動します（再生中も移動できます）。下の枠線付近をドラッグすると波形の縦の高さを変えられます。赤い縦線付近をドラッグすると再生位置を移動できます（再生中も操作できます）。金枠のキュー同士の間に出る白いブロックを右クリックするか Alt+クリックすると、その先のキューへの立ち位置の入り方を選べます。";

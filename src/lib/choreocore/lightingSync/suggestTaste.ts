...
function stripSectionLayoutDirectives(note: string | undefined): string {
  ...
  return note
    .replace(
      /(?:サビ|大サビ|Aメロ|エーメロ|Bメロ|ビーメロ|プレサビ|最初|最後|はじめ|初め|おわり|イントロ|アウトロ)[^。．\n;]{0,48}(?:逆?ピラミッド|V字|ブイ字|ブイ|千鳥|円|扇|W字|グリッド|ばらけ|ダイヤ(?:モンド)?|楔|ワイド|二列|[2２]列)[^。．\n;]*/gi,
      " "
    )
    .replace(
      /(?:逆?ピラミッド|V字|ブイ字|千鳥|円|扇|W字|グリッド|ばらけ|ダイヤ(?:モンド)?|楔|二列|[2２]列)[^。．\n;]{0,24}(?:サビ|大サビ|Aメロ|Bメロ|最初|最後|初め)/gi,
      " "
    );
}

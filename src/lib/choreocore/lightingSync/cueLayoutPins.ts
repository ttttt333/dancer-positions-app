/**
 * 制作者メモ（初回指示・フィードバック）から「最初／最後はピラミッド」など Cue 位置ピンを解析する。
 */

export type CueLayoutSlot = "first" | "last" | "intro" | "outro";
...
/**
 * 「最初と最後はピラミッド」「ラストはV字」などを CueLayoutPin に変換。
 */
...
export function resolvePinnedLayoutForCue(input: {
  ...
  const { pins, cueIndex, cueCount, reasonCodes = [], sectionLabel } = input;

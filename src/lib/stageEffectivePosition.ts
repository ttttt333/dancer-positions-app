...
export type StagePositionOverlays = {
  ...
  tidyPreviewById?: ReadonlyMap<string, StagePosPct> | null;
  groupPosDraft?: ReadonlyMap<string, StagePosPct> | null;
};

/**
 * ステージ上の実効座標。合成はここだけ。
 * shape preview → depth preview → 位置交換 preview → 整える preview → 回転ドラフト → 永続 xPct/yPct
 */
export function getEffectiveDancerPosition(
  ...
  const tidy = overlays.tidyPreviewById?.get(dancer.id);
  if (tidy) return tidy;

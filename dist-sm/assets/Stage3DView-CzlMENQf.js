import { C as Controls, V as Vector3, M as MOUSE, T as TOUCH, Q as Quaternion, S as Spherical, a as Vector2, R as Ray, P as Plane, b as MathUtils, r as reactExports, c as Scene, d as Color, e as PerspectiveCamera, W as WebGLRenderer, D as DirectionalLight, A as AmbientLight, f as PlaneGeometry, g as MeshStandardMaterial, h as Mesh, i as SphereGeometry, j as DANCER_COLOR_PALETTE_THREE, m as modDancerColorIndex, k as DEFAULT_DANCER_MARKER_DIAMETER_PX, l as jsxRuntimeExports, n as MARKER_DIAMETER_PX_MIN, o as MARKER_DIAMETER_PX_MAX } from "./index-CsfdzX0x.js";
...
function Stage3DView({
  ...
  reactExports.useEffect(() => {
    ...
    scene.background = new Color(132631);
    ...
    const dl = new DirectionalLight(16777215, 0.95);
    dl.position.set(3, 18, 8);
    scene.add(dl);
    scene.add(new AmbientLight(6583435, 0.5));
    const planeGeom = new PlaneGeometry(10, 7.5);
    ...
    scene.add(plane);
    const geom = new SphereGeometry(0.28, 20, 20);
    const meshes = [];
    let raf = 0;
    const loop = () => {
      controls.update();
  ...
  reactExports.useEffect(() => {
    ...
    const r0 = 0.28 * (clampD / DEFAULT_DANCER_MARKER_DIAMETER_PX);
    api.geom.dispose();
    const geom = new SphereGeometry(r0, 20, 20);
    api.geom = geom;
    dancers.forEach((d) => {
      const mat = new MeshStandardMaterial({
        color: DANCER_COLOR_PALETTE_THREE[modDancerColorIndex(d.colorIndex)],
        roughness: 0.35,
        metalness: 0.12
      });
...
//# sourceMappingURL=Stage3DView-CzlMENQf.js.map
